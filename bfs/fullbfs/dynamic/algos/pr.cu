...
__global__ void gather_pr_edge(const shard_t * shard, vertex_t * vertices, double * updates, int num_vertices, int current_depth)
{
    ...
    if(id < shard->E)
    {
        ...
        updates[d*num_vertices + s] = u;
    }
}

__global__ void apply_pr_edge(const shard_t * shard, vertex_t * vertices, double * updates, int num_vertices, int current_depth)
{
    int id = blockDim.x*blockIdx.x + threadIdx.x;
    ...
    if(vid <= shard->Vend)
    {
        ...
        {
            sum += updates[vid*num_vertices + shard->from[i]];
        }
        if(fabs(vertices[vid].val - sum) >= ERR)
        {
            d_over=true;
        }
        vertices[vid].val = 0.15 + 0.85 * sum;
    }
}
...
int main(int argc, char * argv[])
{
    ...
    //Update matrix
    double * updates;
    CUDA_SAFE_CALL(cudaMalloc((void **)&updates, num_vertices*num_vertices*sizeof(double)));
        ...
        for(i=0; i<num_intervals; i++)
        {
            if(pingpong==0)
            {
                ...
                if(i>0)
                {
                    ...
                    gather_pr_edge<<<grid, threads,0,str[1]>>> (shard_dev2, vertices, updates,num_vertices, k);
            ...
            else
            {
                ...
                gather_pr_edge<<<grid, threads,0,str[0]>>> (shard_dev, vertices, updates,num_vertices, k);
        ...
        for(i=0; i<num_intervals; i++)
        {
            if(pingpong==0)
            {
                ...
                if(i>0)
                {
                    //Process Pong
                    CUDA_SAFE_CALL(cudaEventRecord(start[1],str[1]));
                    apply_pr_edge<<<grid, threads,0,str[1]>>> (shard_dev2, vertices, updates,num_vertices, k);
                    CUDA_SAFE_CALL(cudaStreamSynchronize(str[1]));
                    CUDA_SAFE_CALL(cudaEventRecord(stop[1],str[1]));
                    CUDA_SAFE_CALL(cudaEventSynchronize(stop[1]));
                    CUDA_SAFE_CALL(cudaEventElapsedTime(&diff[i-1],start[1],stop[1]));
            ...
            else
            {
                ...
                //Process Pong
                CUDA_SAFE_CALL(cudaEventRecord(start[0],str[0]));
                apply_pr_edge<<<grid, threads,0,str[0]>>> (shard_dev, vertices, updates,num_vertices, k);
                CUDA_SAFE_CALL(cudaStreamSynchronize(str[0]));
                CUDA_SAFE_CALL(cudaEventRecord(stop[0],str[0]));
                CUDA_SAFE_CALL(cudaEventSynchronize(stop[0]));
                CUDA_SAFE_CALL(cudaEventElapsedTime(&diff[i-1],start[0],stop[0]));
