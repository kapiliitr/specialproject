...
int main(int argc, char * argv[])
{
	...
	vertex_t * from_dev;
	vertex_t * to_dev;
	CUDA_SAFE_CALL(cudaHostAlloc((void **)&shard, sizeof(shard_t),cudaHostAllocWriteCombined));
	CUDA_SAFE_CALL(cudaHostAlloc((void **)&from_dev, MAX_NUM_EDGES_SHARD*sizeof(edge_t),cudaHostAllocMapped | cudaHostAllocWriteCombined));
	CUDA_SAFE_CALL(cudaHostAlloc((void **)&to_dev, MAX_NUM_EDGES_SHARD*sizeof(edge_t),cudaHostAllocMapped | cudaHostAllocWriteCombined));
	...
	do
	{
		...
		for(i=0; i<num_intervals; i++)
		{
			...
			CUDA_SAFE_CALL(cudaMemcpy(from_dev, shard_host[i].from, shard_host[i].E*sizeof(vertex_t),cudaMemcpyHostToDevice));
			CUDA_SAFE_CALL(cudaMemcpy(to_dev, shard_host[i].to, shard_host[i].E*sizeof(vertex_t),cudaMemcpyHostToDevice));
			CUDA_SAFE_CALL(cudaMemcpy(&(shard->from), &from_dev, sizeof(vertex_t *),cudaMemcpyHostToDevice));
	...
	CUDA_SAFE_CALL(cudaFreeHost(from_dev));
	CUDA_SAFE_CALL(cudaFreeHost(to_dev));
	CUDA_SAFE_CALL(cudaFreeHost(shard));
