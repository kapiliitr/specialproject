...
bool destSort(pair<int,int> i,pair<int,int> j) { return ((i.second<j.second) || (i.second==j.second && i.first<j.first)); }
...
__global__ void bfs(const graph_t * graph, int * vertices)
{
	int id = blockDim.x*blockIdx.x + threadIdx.x;
	if(id < graph->V)
	{
		if(vertices[id] == -1)
		{
			...
			{
				int t=vertices[graph->adj[i]];
				if(t < minLevel)
					minLevel = t;
			}
			if(minLevel >= 0)
			{
				vertices[id] = minLevel+1;
				d_over = true;
			}
		}
	}
...
int main(int argc, char * argv[])
{
	...
	sort(edges.begin(),edges.end(),destSort);

	/*
	   At this point, the vertices and edges are sorted by destination.
	 */
	i=0;
	int ps=edges[0].second,idf,last;
	for(vector< pair<int,int> >::iterator it = edges.begin() ; it != edges.end(); ++it)
	{
		int id = distance(vertices.begin(),vertices.find((*it).first)); // C++ set stores in sorted order by default
		//Storing in-edges instead of out-edges
		graph_host->adj[i] = id;
		if(ps != (*it).second)
		{
			idf = distance(vertices.begin(),vertices.find(ps));
			last = distance(vertices.begin(),vertices.find((*it).second));
			for(j=idf;j<last;j++)
	...
	*****************************************************/
	...
	do
	{
		...
		bfs<<<grid, threads>>> (graph_host, vertices_host);
		CUDA_SAFE_CALL(cudaDeviceSynchronize());
