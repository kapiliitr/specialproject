...
int main(int argc, char * argv[])
{
	...
	for(i=0; i<num_intervals; i++)
	{
		...
		shard_host[i].edges = (edge_t *) malloc(shard_host[i].E*sizeof(edge_t));

		if(shard_host[i].E > MAX_NUM_EDGES_SHARD)
			MAX_NUM_EDGES_SHARD = shard_host[i].E;
	}
...
/*	CUDA_SAFE_CALL(cudaMemcpy(vertices_host, vertices, num_vertices*sizeof(vertex_t), cudaMemcpyDeviceToHost));
	...
	//CUDA_SAFE_CALL(cudaFree(shard->edges));
