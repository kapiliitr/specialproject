...
#ifndef __GATHER__
#define __GATHER__
__global__ void gather(const shard_t * shard, vertex_t * vertices, int * frontier, int num_vertices, int current_depth)
{
}
#endif
 */
...
__global__ void apply(const shard_t * shard, vertex_t * vertices, bool * frontier_cur, bool * frontier_next, int num_vertices, int current_depth)
{
    ...
    if(vid <= shard->Vend)
    {
        if(vertices[vid].val == -1 && frontier_cur[vid] == true)
        {   
            vertices[vid].val = current_depth;
            int i;
            if(id==0)
                i=0;
            else
...
#ifndef __SCATTER__
#define __SCATTER__
__global__ void scatter(const shard_t * shard, vertex_t * vertices, int * frontier, int num_vertices, int current_depth)
{
}
#endif
 */
